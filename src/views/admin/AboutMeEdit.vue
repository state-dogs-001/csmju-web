...
export default {
  data() {
    ...
    return {
      ...
      title: "",
      description: "",
      date: date + "/" + (month + 1) + "/" + (year + 543),
    };
  },
  methods: {
    getAbout() {
      this.id = this.$store.state.aboutEdit;
      http.get(`about/id/${this.id}`).then((response) => {
        this.about_array = response.data;
        this.title = this.about_array.Topic;
        this.description = this.about_array.Detail;
      });
    },
    submit() {
      this.v$.$validate();
      if (!this.v$.$error) {
        ...
        http.post(`about/update/${this.id}`, data).then(() => {
